function Avaliacoes() {
    ...
    function exibirConjunto3() {
        ...
        for (let i = 0; i < opc3.length; i++) {
            document.getElementById(opc3[i]).classList.add('fade-out');
            setTimeout(() => {
                document.getElementById(opc3[i]).src = conjunto3[i];
                document.getElementById(opc3[i]).classList.remove('fade-out');
                document.getElementById(opc3[i]).classList.add('fade-in');
                setTimeout(() => document.getElementById(opc3[i]).classList.remove('fade-in'), 500);
            }, 500);
        }
    }
    ...
    function exibirConjunto2() {
        ...
        for (let i = 0; i < opc2.length; i++) {
            document.getElementById(opc2[i]).classList.add('fade-out');
            setTimeout(() => {
                document.getElementById(opc2[i]).src = conjunto2[i];
                document.getElementById(opc2[i]).classList.remove('fade-out');
                document.getElementById(opc2[i]).classList.add('fade-in');
                setTimeout(() => document.getElementById(opc2[i]).classList.remove('fade-in'), 500);
            }, 500);
        }
    }
    ...
    function proximg1() {
        ...
        animateImageChange("imgopc1", imgs1[imgAtual]);
    }
    function imganterior1() {
        imgAtual = (imgAtual <= 0) ? imgs1.length - 1 : imgAtual - 1;
        animateImageChange("imgopc1", imgs1[imgAtual]);
    }
    function animateImageChange(id, src) {
